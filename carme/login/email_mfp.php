<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        ...
        // Verify credentials
            ...
            $sql=mysqli_query($connection,"SELECT * FROM registration WHERE username='$username' and email='$email' ");
            ...
            if(is_array($row))
            {
                if($row['username']== $username && $row['email']== $email)
                {
                    ...
                    $mail->Body .= "<br><br>Your password for CarMe member account is as followed:<br><br><b>" .$_SESSION['password']. "</b>";
                    $mail->Body .= "<br><br>Thank you!<br><br>Warmest Regards,<br><i>CarMe Admin</i>";

                    if($mail->send())
                    {
                        $successMsg = "Password has been sent to your email. Please check it.";
                        echo "<script>alert('$successMsg'); location.href = 'member_forgot_password.php';</script>";
                    }
                    else
                    {
                        $failed = "Password is failed to sent. Please try again.";
                        echo "<script>alert('$failed'); location.href = 'member_forgot_password.php';</script>";
                    }
